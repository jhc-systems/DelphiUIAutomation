...
begin
  ...
  enquiry := TAutomationDesktop.GetDesktopWindow('Form2');
  enquiry.Focus;

  grid := enquiry.GetStringGridByIndex(0);
  writeln ('Value is ' + grid.Value);

  writeln ('Item @ 3-3 is ' +grid.GetItemText(3,3));

  writeln ('Selected is ' + grid.GetSelectedText);

  write ('Column Headers are ');
  grid.PrintColumHeaders;
